...
本章解释 Agent 运行时如何把执行过程暴露给应用和前端。完成后，你应能区分**模型生成 tool call** 与**工具实际执行**两条流，并在真实 `stream_events(version="v3")` 调用中观察文本增量与最终 output。

离线示例用于理解 projection 数据形状；真实 Agent 示例用于观察 live streaming 行为。
...
3. **两条 tool 相关 projection**：`message.tool_calls` 表示模型正在生成调用参数；`stream.tool_calls` 表示系统实际执行与结果。
4. **真实 live streaming**：`agent.stream_events(..., version="v3")` 可同时消费文本增量、工具执行与最终 state。见 [live_stream_events_agent.py](../../../src/agent_learn/frameworks/langchain/10-event-streaming/live_stream_events_agent.py)。
5. **流式脱敏必须在事件离开 run 前完成**：`after_model` 级 state 清理对 live reader 可能太晚。见 [stream_redaction.py](../../../src/agent_learn/frameworks/langchain/10-event-streaming/stream_redaction.py)。
...
python src/agent_learn/frameworks/langchain/10-event-streaming/live_stream_events_agent.py
```

该命令会调用真实 Agent 的 `stream_events(version="v3")`，打印模型文本增量、工具执行摘要与最终答复。环境变量要求与第 7 章相同。
