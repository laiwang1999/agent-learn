...
将来源资料转化为可学习、可运行、可验证的中文教材。面向读者的 Markdown、代码注释与 tool docstring 使用中文；Python 标识符、文件名、测试名、运行时字符串和结构化数据键使用英文。
...
4. 将示例中的 Python 文件名、模块名、类名、函数名、变量名、tool 名称、测试名、输出文字和结构化数据键写成英文；文件名使用英文小写 `snake_case`。只在行内注释、块注释和 tool docstring 中使用中文。
...
- Markdown 叙述、行内注释、块注释和 tool docstring 的业务说明使用中文；必要英文术语首次出现时写成“中文（English）”。
- 不将中文标点、中文变量名或中文输出混入 Python 标识符、JSON 键、环境变量、URL 或运行时字符串。
...
- 自定义代码示例使用英文语义命名、英文运行时文本和英文结构化数据键，并为关键教学步骤提供中文注释。
